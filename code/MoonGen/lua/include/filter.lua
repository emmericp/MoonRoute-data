...
-- FIXME: add proto mask!!
ffi.cdef [[
struct mg_5tuple_rule {
    uint8_t proto;        // protocol field in the IPv4 header
    uint32_t ip_src;
...
-- One array of values can be bound to multiple categories. After classification
...
--  a new Array will be allocated (with a size equalling the given number) and bound to the specified category.
...
-- @category_mask bitmask for the categories, this rule should be part of
--    (one rule can be valid for multiple categories)
