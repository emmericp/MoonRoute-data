...
function lpm.mg_lpm4Table:addRoutesFromTable(routes, ports, arpLookupFun)
  ...
  for i, route in pairs(routes) do
    ...
    entry.interface = ports[route.nhPort].outputSeqNr
...
-- @param virtualDevices is a table with the virtual device names as indices
--  and the port names as values
function getLinuxRoutes(virtualDevices)
  local rtResult = io.popen("ip route", "r")
  local routes = {}
  repeat
    line = rtResult:read()
    -- zebra routing entries have the form:
    -- 66.76.45.0/24 via 172.16.0.1 dev eth-mgmt  proto zebra
    if(line) then
      printf(line)
      local netIp, netPrefix, gw, dev = line:match("^%s*(%d*%.%d*%.%d*%.%d*)/(%d*)%s+via%s+(%d*%.%d*%.%d*%.%d*)%s+dev%s+([^%s]*)%s+")
      --local netIp, netMsk, gw, dev = line:match("^%s*(%d*%.%d*%.%d*%.%d*)/(%d*)")
      if(netIp and virtualDevices[dev]) then
        local idx = "linux_" .. tostring(#routes+1)
        routes[idx] = {} 
        routes[idx]["nhPort"]         = virtualDevices[dev]
        routes[idx]["nhIPv4"]         = gw
        routes[idx]["networkIP"]      = netIp
        routes[idx]["networkPrefix"]  = tonumber(netPrefix)
      end
      --printf("network: %s, msk: %s, gw: %s, dev: %s", netIp, netPrefix, gw, dev)
    end
  until line == nil
